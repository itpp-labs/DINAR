...
def parse_depfile(depfile):
    ...
    for line in depfile:
        ...
        parts = line.split()
        num_parts = len(parts)
        repo, url, branch, commit = [num_parts > i and parts[i] for i in range(4)]
...
""".format(
        repo=repo, origin=url, branch=branch, commit_line=commit_line
    )
...
if __name__ == "__main__":
    ...
    for content, filename in [(addons, addons_filename), (repos, repos_filename)]:
        if content:
